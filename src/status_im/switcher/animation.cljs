...
(defn calculate-home-stack-position []
  (let [{:keys [width height]} (constants/dimensions)
        ...
        top-empty-space        (* empty-space-half-scale height)]
    {:left  (reduce
             (fn [acc stack-id]
     ...
     :top   (+ top-empty-space constants/switcher-height-offset)
...
(defn bottom-tab-on-press [shared-values stack-id]
  (if (= stack-id @selected-stack-id)
    (close-home-stack shared-values)
